...
@RequestMapping("/asserts/tpl/basedata/collection/task")
@RestController
public class TaskController extends BaseException {
    ...
    public BaseRet runnow(@PathVariable("taskid") String taskid, HttpServletRequest request, 
		...
		String operIp = null;
		try {
			operIp = RequestUtil.getRemoteHost(request);
		} catch (Exception e) {
			e.printStackTrace();
		}
       	int status = taskManagerService.runTask(taskid, user, operIp); 
    	ret.setStatus(status);
    	if(Integer.parseInt(TaskComm.SUCCESS_0)==status){
    		ret.setMessage(PromptMessageComm.OPERATION_SUSSESS);
    	}
