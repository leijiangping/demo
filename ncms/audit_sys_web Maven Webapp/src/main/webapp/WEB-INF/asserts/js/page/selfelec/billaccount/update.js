	          ...
	          if(list.length> 0){
	        	  ...
	        	  $("#contracttb tbody").html("");
				    var s ='<tr style=\'text-align: center;\'><input type=\'hidden\' name=\'getRegname\' id="getRegname" value="'
				    	+ list[0].regId +'"/><input type=\'hidden\' name=\'getPregname\' id="getPregname" value="'+list[0].pregId+'"/ ><td  ><input type=\'radio\' name=\'checkbox\'/></td><td color="red">'
				    	...
				    	+endDate +'</td><td>'
				        +state  +'</td><td>'
				    	+ (list[0].priceType== null ? '-' : list[0].priceType==0?'非平峰谷单价':'平峰谷单价') +'</td><td>'
				    	+ (list[0].isIncludeAll==null?'-':list[0].isIncludeAll==0?'否':'是') +'</td></tr>';
				        $('#contracttb').append(s);
				  }
			  }
		  }
	...
	$.post("queryResourceTree" , {"billaccountId":billaccountId , "regId":regId} , function(data){
		...
		$.each(data.Obj,function(i,item){
			...
			if(!item.parentId){
				...
				str += ' data-parent-id="'+item.parentId +'" data-tt-branch="false"><input type="hidden" name="isShare"  value="'+item.baseresource_code+'"><td><span class="file"></span><input type="hidden" name="relId"  value="'+item.id+'"></td>';
				str += 
					'<td>'+baseresource_type+'</td>' +
					'<td>'+item.baseresource_name+'</td>' +
					'<td>电表</td>' +
...
//			console.log('fff');
//			console.log($("#resourcetreetable tr[data-node-id="+pid+"]").hasClass('appes'));
...
function loadContractTableData(billaccountId) {
	...
	var contracttbcontent =$("#contracttb").find('tr td').eq(1).text();//判断有合同
	var isInclude=null;
	if(contracttbcontent==""){
		isInclude=="";
		priceType="";
		elecontractId="";
	}else{
		if(resourcess && containElec){
			isInclude=$("#isIncludeAll").val();
		...
		if(contracttbids!=null && contracttbids.length>0){
			elecontractId=contracttbids[0].elecontractId;
		}
...
function loadResourceTableData(billaccountId , regId) {
	var oldResourceId="";
	var oldResourceIdlist=$("#resourcetreetable").find('tr');
	
	for(var i=1;i<oldResourceIdlist.length;i++){
		var oldResourceIds=$(oldResourceIdlist[i]).attr('data-node-id');
		if (oldResourceIds != null){
			oldResourceId += oldResourceIds+",";
		}
	}
	// 先销毁表格
...
//	            billaccountId :	billaccountId,
	            ...
	            souArr : oldResourceId,//oldResourceId
	            regId :	regId
			};
			return param;
		},
...
function loadElectricmeterTableData(billaccountId , regId , resourceId) {
	 ...
	 */ 
	 ...
	 var olddbId="";
	 var olddbIdlist =$("#resourcetreetable").find('tr');
		for(var i=0;i<olddbIdlist.length;i++){
			var olddbIs=$(olddbIdlist[i]).attr('data-node-id');
			if (olddbIs != null){
				olddbId+=olddbIs+",";
			}
		}
...
//	 delContractData=contracttbids[0];
	...
	$("#isIncludeAll").val(contracttbids[0].isIncludeAll);
    var s ='<tr style=\'text-align: center;\'><input type=\'hidden\' name=\'checkbox\'/ ><input type=\'hidden\' name=\'getRegname\' id="getRegname" value="'
    	+ contracttbids[0].regId +'"/><input type=\'hidden\' name=\'getPregname\' id="getPregname" value="'+contracttbids[0].pregId +'"/><td ><input type=\'radio\' name=\'checkbox\'/ ></td><td>'
        ...
        + contractState  +'</td><td>'
    	+ (contracttbids[0].priceType== null ? "-" : contracttbids[0].priceType==0?"非平峰谷单价":"平峰谷单价")  +'</td><td>'
		+ (contracttbids[0].isIncludeAll==null?"-":contracttbids[0].isIncludeAll==0?"否":"是") +'</td></tr>';
        $('#contracttb').append(s);
...
function relationContractSubmit(){
	var contract =$("#contracttb").find('tr td').eq(1).text();
	if(delContractData!=null){
		if(contract==null||contract==""){
			relationContractCancel();
		}
	}else{
		if(contract!=null&&contract!=""){
			relationContractUpdate();
		}
	}
}
 ...
 * 关联合同 新增修改
 */
function relationContractUpdate(){
 ...
 * 关联合同 取消
 */
function relationContractCancel(){
	var contract =$("#contracttb").find('tr td').eq(1).text();
	if(delContractData!=null&&contract==null||contract==""){
		$.ajax({
			type : "POST",
			url : 'relationContractCancel',
			data:{"elecontractId":delContractData.contractId ,"elebillaccountcontractId":delContractData.elebillaccountcontractId, "billaccountId":billaccountId},
			dataType : 'json',
			cache : false,
			success : function(data) {
			}
...
function deselcontractPopup(){
	  $("input[type=radio][name='checkbox']:checked").each(function(){
		 $(this).parents("tr").remove();
	   });
	  $("#priceType").val("");
		$("#isIncludeAll").val("");
...
function isChecked(){
	...
	var checklist = $("#relationElectricmetertb tbody input[type='checkbox']");
	for(var i=0;i<checklist.length;i++)
    {
		// 已选中可操作行
	    if(checklist[i].checked == 1){
	    	checkNum ++;
	    	rowschecked.push(tableChecked[i]);
	    }
    } 
	return checkNum;
}
...
function hadCheckedRowData(){
	if(tableChecked==null || !isChecked()){
...
function relationElectricmeter(){
	...
	if(hadCheckedRowData()==false){
		return false;
	}
	 Electricmeterids = rowschecked;
...
//				放在对应的父级tr之后
				$(this).after(html);
				$("#resourcetreetable tr[data-parent-id="+pid+"]").addClass('expanded').css('display','table-row');
				$("#resourcetreetable tr[data-node-id="+pid+"]").addClass('expanded').removeClass('collapsed');
				$(this).find('.indenter').on('click',function(){
					
					if($("#resourcetreetable tr[data-node-id="+pid+"]").hasClass('init')==true){
						if($("#resourcetreetable tr[data-node-id="+pid+"]").hasClass('expanded')==true){
							$("#resourcetreetable tr[data-node-id="+pid+"]").addClass('expanded').removeClass('collapsed');
							$("#resourcetreetable tr[data-parent-id="+pid+"]").css('display','table-row');
						}else{
...
function save(){
	...
	if(resourcess!=null && resourcess!=""){//判断是否有资源
		...
		//判断是否关联合同
		if(contract != ""){
			var bzdPreg=$('#modifyCity option:selected').val();//报账点地市
			var bzdReg=$('#modifyRegion option:selected').val();
...
function checkht(){
	//选中合同
	var contract =$("#contracttb").find('tr td').eq(1).text();
	var myselect=$("#contracttb").find('tbody tr');
	if(contract){
		myselect.click(function(e){
			e.stopPropagation(e);
			var inpzt=$(this).find('input:radio[name="checkbox"]:checked').val();
			if(inpzt==undefined){
		...
		$("#contracttb").find('tbody tr input[type="radio"]').on('click',function(e){
			e.stopPropagation();
		})
	}
...
//选中机房
function checkJi(){
		var resourcetb =$("#resourcetreetable").find('tr td').eq(1).text();
		var myresourcetbselect=$("#resourcetreetable").find('tbody tr');
			myresourcetbselect.click(function(e){
				var inpzt=$(this).find('input:checkbox[name="resourceCk"]').prop( "checked");
				if(inpzt==true){
					$(this).find("input[name='resourceCk']").prop( "checked", false );
				}else{
					$(this).find("input[name='resourceCk']").prop( "checked", true );
				}
				e.stopPropagation();
			});
			
			$("#resourcetreetable").find('tbody tr input[type="checkbox"]').on('click',function(e){
				e.stopPropagation();
			})
