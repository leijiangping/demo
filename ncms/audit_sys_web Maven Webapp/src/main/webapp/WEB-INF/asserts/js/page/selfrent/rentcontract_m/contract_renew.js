...
function back(){
	renewFlag = localStorage.getItem("renewFlag");
	if(renewFlag != 1){
...
function checkContractEndDate(){	
	...
	if(start>end){
		$("#contractEnddate").next("#err").html("<img src=\"../../../image/error.png\"/>合同期终需大于期始！");
		$("#contractEnddate").next("#err").css({"color":"red"});
	}else{
		...
		success : function(back) {
			if(back != null){
				if(back.success == "1"){
					...
					$("#contractStartdate").val(new Date(rentContractVO.datContractVO.contractEnddate).format("yyyy-MM-dd"));
