...
function elecAccountRecordPopup(){
	...
	if(contractFlag){
		alertModel("此合同状态为非正常，不能进行缴费！");
		return;
	}
	if(auditingFlag){
		alertModel("此合同审核未通过，不能进行缴费！");
		return;
	}
...
function elecAccountAllPopup(){
	window.parent.addIframeTop("asserts/tpl/selfelec/payment/manage.html","1000","报账点电费明细维护");
//	window.location.href="manage.html?elecback="+1;
}
