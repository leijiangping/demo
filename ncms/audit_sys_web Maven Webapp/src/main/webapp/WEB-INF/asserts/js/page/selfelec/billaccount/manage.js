...
function submitApproval(){
    ...
    var issss = $('#tb') .bootstrapTable('getAllSelections') ;
    showBenchmark();
    var airconditionerPowerTotle=0;
    var basestationPowerTotle=0;
    var dayBenchmark=0;
    var airconditionerPowerTime='';
    if(calBenchmark!=null){
    	console.log(calBenchmark);
    	//报账点空调总功率为：**千瓦
    	airconditionerPowerTotle=calBenchmark[0].airconditionerPower==null?0:calBenchmark[0].airconditionerPower;
    	//设备总功率为：**千瓦
    	if(calBenchmark[0].datBasestationVO[0]!=null){
    		basestationPowerTotle=calBenchmark[0].datBasestationVO[0].basestationPower;
    	}
    	//日额定功率标杆=空调总功率*空调运行时长+设备总功率*24。
    	if(calBenchmark[0].eleBenchmark!=null){
    		airconditionerPowerTime=calBenchmark[0].eleBenchmark.t;
    	}
    	//日额定功率标杆为**度
    	if(basestationPowerTotle!=null && airconditionerPowerTime!=''){
    		dayBenchmark=airconditionerPowerTotle*airconditionerPowerTime+basestationPowerTotle*24
    	}
    }
    confirmModel('报账点空调总功率为：'+airconditionerPowerTotle+'千瓦，设备总功率为：'+basestationPowerTotle+'千瓦，日额定功率标杆为：'+dayBenchmark+'度。系统需要用该值填补2015年1月1号至今的额定功率标杆历史值，请确认该标杆值是否准确。（如果认为该标杆值不准确，请点击“取消”，并在资管系统修改相应功率数据，待系统重新采集新的功率数据后，再次提交审核）?','submitCheck');
    
}
var calBenchmark=null;
function showBenchmark(){
	 var ids = $('#tb') .bootstrapTable('getAllSelections') ;
    $.ajax({
        url : 'queryBenchmark',// 跳转到 action    
        data : {
        	billaccountId:ids[0].billaccountId,
        	regId:ids[0].pregId
        },
        type : 'post',
        dataType : 'json',
