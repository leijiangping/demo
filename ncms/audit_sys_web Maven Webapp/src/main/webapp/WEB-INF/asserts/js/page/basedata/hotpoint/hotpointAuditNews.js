...
function initCurrentPage(){
	queryCurUser();
	//下级审核人
	var id=getParameter("baseresourceId");
	var regId=getParameter("regId");
	findUsersByRoleIds(BaseResourceAudit.tableName,id,regId);
	//流转记录
	histoicFlowList(BaseResourceAudit.tableName,id);
}
...
function back(){
	window.location.href='hotpointAudit.html';
}
function changeSave(){
...
function auditNewsFormSubmit(){
	...
	var obj={
    	...
    	"auditNote":auditNote,
    	"taskId":taskId,
    	"id":getParameter("baseresourceId")
    };
	ids.push(obj);
