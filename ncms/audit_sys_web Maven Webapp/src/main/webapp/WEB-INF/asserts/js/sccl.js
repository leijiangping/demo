...
$(".side-menu").on('click', 'li a', function(e) {
    ...
    if (checkElement.is('.menu-item-child') && checkElement.is(':visible')) {// 已经 有二级菜单并可见
      ...
      if(checkElement.parent().parent().hasClass('side-menu')==false){
          ...
          checkElement.parents(".menu-item-child").find('.sign img').attr('src',""+projectName+"/asserts/image/xiao.png");
          ...
          $(checkElement.parents('.menu-item').find('img').get(0)).attr('src',""+projectName+"/asserts/image/"+newParImg+".png");
      }else{
          checkElement.slideUp(animationSpeed, function() {
                checkElement.find('.menu-item-child').removeClass('menu-open').hide();
                checkElement.removeClass('menu-open');
              });
            checkElement.find("li").removeClass("active");
              ...
              checkElement.prev().find('img').attr('src',""+projectName+"/asserts/image/"+newParImg+".png");
      }
          ...
          for(var s=0; s<menus.length; s++){   
            var item = menus[s];
                if(item.name==oldName){
                    if(item.childMenus){
                        $("#"+oldId).parent().children('img').attr('src',""+projectName+"/asserts/image/"+item.icon+".png");
                    }
                }
            }
        }
      ...
      checkElement.slideDown(animationSpeed, function() {
        ...
        if(parent_li.hasClass('menu-item')){
            if(parent_li.parents('.menu-item').find('.menu-item-child').eq(0).css('display')=='block'){
                ...
                $(parent_li.parents('.menu-item').find('img').get(0)).attr('src',""+projectName+"/asserts/image/"+item.icon+"-active.png");
            }else{
                $(parent_li.parents('.menu-item').find('img').get(0)).attr('src',""+projectName+"/asserts/image/"+item.icon+".png");
            }
            $(parent_li).find('ul img').attr('src',""+projectName+"/asserts/image/xiao.png");
        }else{
            if(parent_li.hasClass('degre2')){
                $(parent_li.find('img').get(0)).attr('src',""+projectName+"/asserts/image/xiao-active.png");
                var otherImg=$(parent_li).siblings();
                for(var i=0;i<otherImg.length;i++){
                    $(otherImg.eq(i).find('img').get(0)).attr('src',""+projectName+"/asserts/image/xiao.png");
                    otherImg.eq(i).removeClass('active');
                    $(otherImg.eq(i).find('.degre2').get(0)).removeClass(' menu-open').hide();
                    $($(otherImg.eq(i).find('.degre2').get(0)).find('.degre3 img').get(0)).attr('src',""+projectName+"/asserts/image/xiao.png");
                    
                    if(otherImg.eq(i).find('.degre3').length>0){
                        $(otherImg.eq(i).find('.degre3').find('img').get(0)).attr('src',""+projectName+"/asserts/image/xiao.png");
                    }
                }
            }
            ...
            if(parent_li.hasClass('degre3')){
                ...
                for(var i=0;i<otherImg.length;i++){
                    ...
                    if(otherImg.eq(i).find('.degre4').length>0){
                        $(otherImg.eq(i).find('.degre4').find('img').get(0)).attr('src',""+projectName+"/asserts/image/xiao.png");
                    }
                }
            }
        }
        ...
        if(item.name==$this.children("span").text()){
            ...
            setTimeout(function(){
                if($this.parents('.menu-item').find('.menu-item-child').eq(0).css('display')=='block'){
                    ...
                    $this.children('img').attr('src',""+projectName+"/asserts/image/"+item.icon+"-active.png");
                }else{
                    $this.children('img').attr('src',""+projectName+"/asserts/image/"+item.icon+".png");
                }
            },500)
            }else{
                $this.children('img').attr('src',""+projectName+"/asserts/image/"+item.icon+"-active.png");
...
//          oldName=item.name;
            oldId=item.id;
            $this.append("<input type='hidden' id="+item.id+" />");
            ...
            if(item.isHeader == "1"){
                ...
                if(item.childMenus == ""){
                    if(item.icon==''){
                        ...
                        str = "<li><a onclick='openWin(this,\""+item.url+"\",\""+item.id+"\")'><span>"+item.name+"</span></a></li>";
                    }else{
                        if(item.icon=='xiao'&&item.parentId.length=='5'){
                            str = "<li><a class='level3' onclick='openWin(this,\""+item.url+"\",\""+item.id+"\")'><img src=\""+projectName+"/asserts/image/"+item.icon+".png\"/><span>"+item.name+"</span></a></li>";

                        }else if(item.id.length=='5'){
                            str = "<li><a onclick='openWin(this,\""+item.url+"\",\""+item.id+"\")'><img src=\""+projectName+"/asserts/image/"+item.icon+".png\"/><span>"+item.name+"</span></a></li>";
...
//                  }
                    if(item.code.length=='7'){
                        leve_class='degre2';
                    }else if(item.code.length=='9'){
                        leve_class='degre3';
                    }else if(item.code.length=='11'){
                        leve_class='degre4';
                    }else if(item.code.length=='13'){
                        leve_class='degre5';
                    }
                    str = "<li class='"+leve_class+"'><a class='sign'><img src=\""+projectName+"/asserts/image/"+item.icon+".png\"/><span>"+item.name+"</span></a>";
                    str +="<ul class='menu-item-child "+leve_class+"' id='menu-child-"+item.id+"'></ul></li>";
                    $(parent).append(str);
...
function openWin(obj, url, menuId,icon)
{
    ...
    if(curSrc!=='undefined'&&curSrc!==undefined){
        ...
        $(obj).parent().parent().parent().parent().find('.sign img').attr('src',""+projectName+"/asserts/image/xiao.png");
        ...
        $($(obj).parents('.menu-item').find('.sign').get(0)).find("img").attr('src',""+projectName+"/asserts/image/"+newImg+".png");
        }else{
            //没有二级菜单
            $(obj).find('.sign img').attr('src',""+projectName+"/asserts/image/xiao.png");
            var curImg=$($(obj).parents('.menu-item').find('.sign').get(0)).find("img").attr('src').split('image')[1];
            var newImg=curImg.split('/')[1].split('.')[0].split('-')[0];
            $($(obj).parents('.menu-item').find('.sign').get(0)).find("img").attr('src',""+projectName+"/asserts/image/"+newImg+".png");
        }
...
/*$(function(){
    ...
    var menu = [{"id":"1","name":"主菜单","parentId":"0","url":"","icon":"","order":"1","isHeader":"1","childMenus":[
                    {"id":"3","name":"商品管理","parentId":"1","url":"","icon":"&#xe604;","order":"1","isHeader":"0","childMenus":[
                        {"id":"4","name":"品牌管理","parentId":"3","url":"asserts/tpl/test1.html","icon":"","order":"1","isHeader":"0","childMenus":""},
                        {"id":"5","name":"分类管理","parentId":"3","url":"asserts/tpl/test2.html","icon":"","order":"1","isHeader":"0","childMenus":""}
                    ]},
                    {"id":"6","name":"订单管理","parentId":"1","url":"","icon":"&#xe602;","order":"1","isHeader":"0","childMenus":[
                        {"id":"7","name":"已付款","parentId":"6","url":"asserts/tpl/home3.html","icon":"","order":"1","isHeader":"0","childMenus":""},
                        {"id":"8","name":"未付款","parentId":"6","url":"asserts/tpl/home4.html","icon":"","order":"1","isHeader":"0","childMenus":""}
                    ]}
                ]},
