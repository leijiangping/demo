...
function updateContract(type){
	...
	if(rows[0].contractState != 0 && rows[0].contractState != 4){
		if(dataFrom == 0){
			alertModel("此固化信息状态不符合修改条件，无法修改！");
		}else{
			alertModel("此合同状态不符合补录条件，无法补录！");
		}
		return;
	}
...
//	var rows = getSelectItem();
	...
	for (var i = 0; i < rows.length; i++) {
		if(rows[i].auditingState == 9){
			alertModel("所选数据有已经进入流程，无法删除！");
			return;
		}
		if(rows[i].auditingState == 0){
			alertModel("所选数据有已审核通过，无法删除！");
			return;
		}
//		delete rows[i]["0"];
	}
...
//	var submitData = JSON.stringify(rows);
	if (confirm("确定删除?")) {
		...
		$.each(rows, function (i, n){
			contractIds[contractIds.length] = n.contractId;
		});
