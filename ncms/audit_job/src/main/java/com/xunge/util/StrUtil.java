...
import com.xunge.controller.job.ContractRestController;
...
public class StrUtil {
	...
	/**
	 * CSV文件校验处理   aaa|"AAA|FFF"|FFFF  =  {aaa,"AAA|FFF",FFF} 
	 * @param val 字符串  如： aaa|"AAA|FFF"|FFFF
	 * @param delimiter 分隔符  如：|
	 * @return 00001
	 */
	public static String[] CSVtoArray(String val,String delimiter){
		...
		String[] vals = val.split(delimiter,-1);
		boolean flag = false;
		int index = 0;
		int num = 0;
		...
		for(int i= 0;i<vals.length;i++){
			...
			if(!flag){
				if(vals[i].startsWith("\"")&&vals[i].endsWith("\"")){
					list.add(index++,vals[i]);
				}
				else if(vals[i].startsWith("\"")&&is){
					list.add(index,vals[i].replace("\"", ""));
					num++;
					flag = true;
				}else{
					list.add(index++,vals[i]);
				...
				if(is&&num==0&&vals[i].startsWith("\"")){
					list.add(index,vals[i].replace("\"", ""));
					num++;
					flag = true;
				}else if(is&&num!=0&&!vals[i].endsWith("\"")){
					list.set(index, list.get(index)+delimiter.replaceAll("\\\\","")+vals[i]);
				}else{
					list.set(index, list.get(index)+delimiter.replaceAll("\\\\","")+vals[i].replace("\"", ""));
					index++;
					num = 0;
					flag = false;
				}
