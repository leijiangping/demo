...
public class CollectionUtil {
	 ...
	 * @return
	 */
	public static <E> List<E> getDifferentList(List<E> listOld,List<E> listNew){
		List<E> listResult=new ArrayList<E>();
		for (int i = 0, length = listOld.size(); i<length; i++) {	
			if (!listNew.contains(listOld.get(i))){
				listResult.add(listOld.get(i));
			}
		}
		return listResult;
	}
	 ...
	 * @return
	 */
	public static <E> List<E> getEqualList(List<E> listOld,List<E> listNew){
		List<E> listResult=new ArrayList<E>();
		for (int i = 0, length = listOld.size(); i<length; i++) {	
			if (listNew.contains(listOld.get(i))){
				listResult.add(listOld.get(i));
	...
	public static boolean isContain(List<String> list, String str){
		...
		for(String liststr : list){
			if(liststr.startsWith(str) || str.startsWith(liststr)){
				return true;
			}
			else if(liststr.endsWith(str) || str.endsWith(liststr)){
				return true;
			}
			else{
				flag = false;
