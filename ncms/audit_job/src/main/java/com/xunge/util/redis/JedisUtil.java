...
public class JedisUtil {
	...
	static {
		...
		int maxTotle = Integer.parseInt(prop.getProperty("redis.maxTotle"));
		...
		Boolean testOnBorrow = Boolean.parseBoolean(prop
				.getProperty("redis.testOnBorrow"));
		config.setTestOnBorrow(testOnBorrow);

		// redis如果设置了密码：
		String REDIS_IP = prop.getProperty("redis.host");
		int REDIS_PORT = Integer.parseInt(prop.getProperty("redis.port"));
		String REDIS_PASSWORD = prop.getProperty("redis.password");
		int TIME_OUT = Integer.parseInt(prop.getProperty("redis.timeout"));
		if(StrUtil.isBlank(REDIS_PASSWORD)){
			// redis未设置了密码：
			jedisPool = new JedisPool(config, REDIS_IP, REDIS_PORT);
		}
		else{
			jedisPool = new JedisPool(config, REDIS_IP, REDIS_PORT, TIME_OUT,
					REDIS_PASSWORD);
		}
		logger.info("------------redis启用---------------------");
	}
	...
	public Jedis getJedis() {
		...
		jedis.select(Integer.parseInt(prop.getProperty("redis.database")));
		return jedis; 
	}
	...
	public class Keys {
		...
		/**
		 * 批量删除keys对应的记录,可以是多个key
		 * 
		 * @param String
		 *            ... keys
		 * @return 删除的记录数
		 * */
		public void batchDel(String keytips){  
			Jedis jedis = getJedis();
	        Set<String> set = jedis.keys("*"+ keytips +"*");  
	...
	public class Sets {
		...
		public long sadd(String key, List<String> members) {
			...
			Jedis jedis = getJedis();
			long s = jedis.sadd(key, (String[])members.toArray(new String[members.size()]));
			returnJedis(jedis);
			return s;
		}
