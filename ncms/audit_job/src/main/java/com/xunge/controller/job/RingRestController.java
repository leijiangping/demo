...
@RestController
@RequestMapping("/restful/ringsystem/")
public class RingRestController {
	private static final Logger LOGGER = Logger.getLogger(RingRestController.class);
	...
	public String syncData(HttpServletRequest request,HttpServletResponse response,@RequestBody TaskInfoVO taskInfo) {
		...
		MultiThread t=new MultiThread(jobSyncService,taskInfo,"ring");
		taskExecutor.execute(t);
		ret.setStatus(0);
		ret.setMessage("同步命令发送成功!");
		LOGGER.info("--------已接收调起任务通知：动环导入");
