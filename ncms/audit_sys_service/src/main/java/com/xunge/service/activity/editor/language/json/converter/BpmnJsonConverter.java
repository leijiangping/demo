 ...
 */
package com.xunge.service.activity.editor.language.json.converter;
...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
	...
	public static final String MODELER_NAMESPACE = PromptMessageComm.MODELER_NAMESPACE;
	protected static final DateFormat defaultFormat = new SimpleDateFormat(SelfelecComm.FORMAT_yyyyMMddHHmmss);
	protected static final DateFormat entFormat = new SimpleDateFormat(SelfelecComm.FORMAT_yyyyMMddHHmmssSSS);
	...
	public ObjectNode convertToJson(BpmnModel model) {
		...
		modelNode.put("resourceId", PromptMessageComm.CANVAS);
		...
		stencilsetNode.put("namespace", PromptMessageComm.NAMESPACE);
		stencilsetNode.put("url", PromptMessageComm.URL);
		...
		if (mainProcess.isExecutable() == false) {
			propertiesNode.put(PROPERTY_PROCESS_EXECUTABLE, PromptMessageComm.NO_EN);
		}
	...
	protected void processFlowElement(FlowElement flowElement, FlowElementsContainer container, BpmnModel model, ArrayNode shapesArrayNode,
		...
		if (converter != null) {
			try {
				...
				LOGGER.error(PromptMessageComm.ERROR_CONVERT, flowElement, e);
			}
		}
	}
		...
		for (Artifact artifact : container.getArtifacts()) {
			...
			if (converter != null) {
				try {
					...
					LOGGER.error(PromptMessageComm.ERROR_CONVERT, artifact, e);
				}
			}
		}
	}
	...
	public BpmnModel convertToBpmnModel(JsonNode modelNode) {
		...
		bpmnModel.setTargetNamespace(PromptMessageComm.TARGET_NAMESPACE);
		...
		if (signalDefinitionNode != null) {
			if (signalDefinitionNode instanceof ArrayNode) {
				...
				while (signalDefinitionIterator.hasNext()) {
					...
					signal.setScope((signalScope.toLowerCase().equals(PromptMessageComm.SCOPE_PROCESS_INSTANCE_LOWER_CASE)) ? Signal.SCOPE_PROCESS_INSTANCE : Signal.SCOPE_GLOBAL);
	...
	public void processJsonElements(JsonNode shapesArrayNode, JsonNode modelNode, BaseElement parentElement, Map<String, JsonNode> shapeMap,
		...
		for (JsonNode shapeNode : shapesArrayNode) {
			...
			try {
				...
				LOGGER.error(PromptMessageComm.ERROR_CONVERT, BpmnJsonConverterUtil.getStencilId(shapeNode), e);
			}
		}
	}
	...
	private void postProcessElements(FlowElementsContainer parentContainer, Collection<FlowElement> flowElementList, Map<String, JsonNode> edgeMap,
		...
		for (FlowElement flowElement : flowElementList) {
			...
			if (flowElement instanceof BoundaryEvent) {
				...
				if (activity == null) {
					LOGGER.warn(PromptMessageComm.BOUNDARY_EVENT + boundaryEvent.getId() + PromptMessageComm.EVENT_NOT_ATTACHED_TO_ACTIVITY);
				} else {
	...
	private void readEdgeDI(Map<String, JsonNode> edgeMap, Map<String, List<JsonNode>> sourceAndTargetMap, BpmnModel bpmnModel) {
		for (String edgeId : edgeMap.keySet()) {
			...
			if (sourceRefNode == null) {
				LOGGER.info(PromptMessageComm.SKIP_EDGE_AND_SOURCE_REF_NULL, edgeId);

				continue;
			}

			if (targetRefNode == null) {
				LOGGER.info(PromptMessageComm.SKIP_EDGE_AND_TARGET_REF_NULL, edgeId);
