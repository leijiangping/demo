...
public class ContractJobServiceImpl implements IContractJobService{
	...
	public boolean dealContract(String url, String dateStr,SysProvinceVO sysProvinceVO) {
		...
		try{
			String baseUrl=url+PromptMessageComm.BASE_URL_SUBMIT_DATE+dateStr+PromptMessageComm.BASE_URL_PAGE_SIZE+pageSize+PromptMessageComm.BASE_URL_CURRENT_PAGE;
	...
	private void callProcedure(String prvId){
		...
		if(0==errCode){
			LOGGER.info(PromptMessageComm.CONTRACT_COMPARE_SUCCESS);
		}
		else if(-1==errCode){
			LOGGER.error(PromptMessageComm.CONTRACT_COMPARE_FAIL);
		}
	}
