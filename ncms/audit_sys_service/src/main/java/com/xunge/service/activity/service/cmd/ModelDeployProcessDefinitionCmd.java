package com.xunge.service.activity.service.cmd;
	...
	public Void execute(CommandContext commandContext) {
		...
		try{
			...
			if (processDefinition != null){
			    ...
			    resource.setName(procDefName + PromptMessageComm.PROCESS_NAME_END_WITH);
				...
				byte[] diagramBytes = IoUtil.readInputStream(processEngineConfiguration.
	                    getProcessDiagramGenerator().generateDiagram(bpmnModel, PromptMessageComm.PNG, processEngineConfiguration.getActivityFontName(),
				...
				diagramResource.setName(procDefName + "." + processDefinition.getKey() + PromptMessageComm.SUFFIX_PNG);
			...
			// 不存在部署一个新的流程 ThinkGem
				...
				repositoryService.createDeployment().name(procDefName).addInputStream(
						procDefName + PromptMessageComm.PROCESS_NAME_END_WITH, new ByteArrayInputStream(bpmnBytes)).deploy();
			}
		}catch(Exception e){
			throw new ActivitiException(PromptMessageComm.DEPLOYMENT_FAIL, e);
		}
		return null;
	}
}